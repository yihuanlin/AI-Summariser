...
// @version      1.0
// @description  Add a summariser to webpages using LLM APIs
// @author       yihuanlin
// @match        *://*/*
// @exclude      *://localhost*
// @exclude      *://127.0.0.1*
...
// @require      https://cdnjs.cloudflare.com/ajax/libs/pdf.js/5.0.375/pdf.min.mjs
// @require      https://cdnjs.cloudflare.com/ajax/libs/pdf.js/5.0.375/pdf.worker.min.mjs
// ==/UserScript==
  ...
  \`\`\`
    ...
    const util = {
        async saveSettings(synced = true) {
            await GM_setValue('summariserSettings', JSON.stringify(state.settings));
        ...
        async loadSettings() {
            try {
                ...
                const result = await GM_getValue('summariserSettings');
        ...
        async saveConversationHistory() {
            ...
            await GM_setValue('summariserHistory', JSON.stringify(state.conversationHistory));
        ...
        async loadConversationHistory() {
            try {
                // Load from browser storage
                const result = await GM_getValue('summariserHistory');
                if (result) {
                    state.conversationHistory = JSON.parse(result);
                } else {
        ...
        async extractPdfContent(pdfUrl) {
            try {
                ...
                if (typeof pdfjsLib === 'undefined') {
                    const pdfjs = await import('https://cdnjs.cloudflare.com/ajax/libs/pdf.js/5.0.375/pdf.min.mjs');
                            ...
                            .then(remoteSettings => {
                                if (remoteSettings && remoteSettings.timestamp) {
                                    ...
                                    if (remoteTimestamp > localTimestamp) {
                                        ...
                                        return GM_setValue('summariserSettings', JSON.stringify(state.settings));
            ...
            reader.onload = function (e) {
                try {
                    ...
                    if (historyData.conversationHistory) {
                        ...
                        GM_setValue('summariserHistory', JSON.stringify(state.conversationHistory));
...
@media (max-width: 600px) {
  :host {
    --summariser-panel-width: calc(100vw - 2px) !important;
  }

  .summariser-resize-handle.summariser-open {
    display: block;
  }
}
...
.summariser-button.left-aligned {
  right: auto;
  left: 8px;
}

.summariser-button-dragging {
  transition: none;
  opacity: 0.8;
}

.summariser-button:hover {
  transform: translateY(-50%) scale(1.05);
}

.summariser-button-icon {
  width: 32px;
  height: 32px;
  fill: var(--summariser-text);
}
...
.summariser-panel {
  position: fixed;
  top: 0;
  right: calc(-1 * var(--summariser-panel-width) - 50px);
  width: var(--summariser-panel-width);
  height: calc(100% - 2px);
  display: flex;
  flex-direction: column;
  pointer-events: auto;
  transition: right var(--summariser-animations-duration) ease;
  z-index: 10000;
}

.summariser-panel.summariser-open {
  right: 0;
}
